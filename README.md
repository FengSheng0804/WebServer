...
**注**：如果觉得编译文件和gz压缩包太多的话，可以运行clean.bat脚本进行删除！！！
...
![服务器GUI的启动2](../docs/images/image-服务器GUI的启动2.png)
...
若浏览器向服务器发送的请求在服务器中查询不到，也就是没有的时候，会给浏览器返回`404 Not Found`状态码。
...
客户端在 HTTP 请求头中可以使用`Accept - Encoding`字段来告知服务器它所支持的压缩算法。例如，`Accept - Encoding: gzip, deflate`表示客户端可以接受 gzip 和 deflate 这两种压缩算法对数据进行压缩。这一步是数据压缩的起始点，通过这个请求头，客户端向服务器传达了它的压缩能力。

#### 服务器响应

服务器在收到客户端的请求后，如果发现客户端支持压缩，并且服务器也有能力对数据进行压缩，它会选择一种合适的压缩算法来压缩响应数据。然后，服务器会在响应头中添加`Content - Encoding`字段，用于标识所使用的压缩算法。例如，如果服务器使用 gzip 算法压缩数据，响应头中会有`Content - Encoding: gzip`。

此时，服务器发送的实际数据是经过压缩后的内容。以一个 HTML 文件为例，如果原始文件大小为 100KB，经过 gzip 压缩后可能只有 30KB 左右，大大减少了数据传输量。

#### 客户端解压

客户端在接收到服务器的响应后，会根据`Content - Encoding`字段中标识的压缩算法来对数据进行解压。如果是 gzip 压缩的，客户端会使用相应的 gzip 解压算法来还原数据，然后再进行后续的处理，比如在浏览器中显示网页内容。
...
- **gzip**
  ...
  gzip 是一种广泛使用的压缩算法。它通过 Lempel - Ziv 编码（LZ77）和哈夫曼编码相结合的方式来压缩数据。LZ77 算法用于查找数据中的重复字符串，然后用一个较短的标记来替换它们。哈夫曼编码则是根据字符出现的频率对字符进行编码，使得出现频率高的字符用较短的编码表示，从而进一步减少数据长度。

  例如，在一个包含大量重复文本的网页（如新闻网站的文章列表页）中，gzip 可以高效地压缩这些重复的 HTML 标签和文本内容，压缩率可以达到 70% 甚至更高。

- **deflate**

  deflate 结合了 LZ77 算法和哈夫曼编码，它与 gzip 类似，但在一些细节上有所不同。deflate 通常比 gzip 在实现上更简单，占用的计算资源相对较少。它在处理一些小型文本文件或者简单的网络协议数据时比较有效。

#### 数据压缩的优势

- **减少网络带宽占用**

  在网络传输过程中，数据量的减少意味着可以更快地传输数据，尤其是在网络带宽有限的情况下。例如，对于移动网络用户或者网络连接不稳定的用户，数据压缩可以使网页加载速度更快，减少等待时间。

- **提高用户体验**

  快速的数据传输能够让客户端更快地接收到完整的信息，如网页、图片、脚本等。以电子商务网站为例，更快的页面加载速度可以提高用户的购物体验，降低购物车放弃率。

## 各种功能的展示

### 代理服务器的代理缓存

运行程序，启动目标服务器和代理服务器。打开浏览器，访问`http:127.0.0.1:10000/web/xtx/index.html`可以在代理服务器中看到来自浏览器的访问请求、向服务器传输数据的情况、Cache的命中情况、Cache的替换情况。在运行第一次时，可以看到，全部的URL请求都不命中，这是因为在此时代理缓存中还没有数据，只有在浏览器发送完一次请求后，才会在代理缓存中添加新的数据。

![代理缓存效果1](./docs/images/image-代理缓存效果1.png)

观察完代理服务器的输出后，我们对网页进行刷新，同时注意目标服务器和代理服务器的输出，我们可以发现，代理服务器的窗口中请求全部命中（这是因为此时缓存没满，一个请求的内容全部被保存进了缓存中），同时，目标服务器的窗口中没有产生任何输出，也就是说此次的请求并没有发送给目标服务器，全部由代理服务器一手接管。

![代理缓存效果2](./docs/images/image-代理缓存效果2.png)

接着，为了体现出缓存的LRU替换算法，我们多加载几个网页，比如`http:127.0.0.1:10000/web/xczx/index.html`，`http:127.0.0.1:10000/web/jswz/index.html`，`http:127.0.0.1:10000/web/kw/index.html`。在观察代理服务器窗口的输出，我们可以看到`Cache is full, removing least recently used item`的字样，表示代理缓存满了，我们使用LRU算法进行替换，将最长时间没使用的缓存删除，将新的数据存进缓存中，也就是实现了LRU算法。

![代理缓存效果3](./docs/images/image-代理缓存效果3.png)

### 数据压缩

在客户端向代理服务器发送请求后，代理服务器转手将请求发送给目标服务器，目标服务器根据请求头查找本地文件，并获取客户端支持的压缩方法，将本地文件通过支持的压缩方法将压缩后的数据或文件发送给客户端，并向客户端发送一个响应头，其中添加服务器使用的压缩方法，在客户端进行解压缩后得到数据。

服务器先获取到客户端支持的压缩方法

![数据压缩效果1](./docs/images/image-数据压缩效果1.png)

如果采用的是Gzip压缩方法的话，输出相关信息，并执行Gzip压缩，将得到的gz文件保存在源文件所在目录下。

![数据压缩效果2](./docs/images/image-数据压缩效果2.png)

![数据压缩效果3](./docs/images/image-数据压缩效果3.png)

如果采用的是Deflate压缩方法的话，输出相关信息，并执行Deflate压缩，Deflate压缩不保存文件，而只是字节数组。

![数据压缩效果4](./docs/images/image-数据压缩效果4.png)

同时在目标服务器的窗口中，我们可以看到相应的输出信息

![数据压缩效果5](./docs/images/image-数据压缩效果5.png)

看`~~~~~`所在行，可以看到请求的文件被采用Gzip方法进行压缩。
