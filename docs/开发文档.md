...
声明用于服务器的 ServerSocket、客户端的 Socket 和读取数据的 bufferedReader。
```java
...
### 创建服务器套接字
使用 ServerSocket 构造器创建服务器套接字，并绑定到指定端口（其中我们指定的是10000）。由于 ServerSocket 构造器可能会抛出异常，因此需要进行异常处理。
```java
try {
...
使用 accept 方法监听指定端口的连接请求。如果没有监听到内容，会一直阻塞线程，直到建立连接成功，返回 clientSocket 对象
...
通过输入流读取 clientSocket 的数据。首先获取输入流，然后使用 InputStreamReader 将字节流转换为字符流，最后将字符流转换为缓冲字符流。
```java
try {
...
在 finally 块中关闭所有打开的资源，包括 BufferedReader、clientSocket 和 serverSocket。
```java
finally {
    if (br != null) {
        try {
...
### 声明必要的变量
在 main 方法中，声明用于客户端的 Socket 和发送数据的 PrintWriter。
```java
Socket clientSocket = null;
PrintWriter pw = null;
```
...
使用 Socket 构造器创建客户端套接字，并连接到指定服务器地址和端口，我们使用本机地址 127.0.0.1 的端口 10000）。由于 Socket 构造器可能会抛出异常，因此需要进行异常处理。
...
通过输出流向服务器端发送数据。首先获取输出流，然后使用 PrintWriter 向服务器端发送数据。
```java
try {
...
在 finally 块中关闭所有打开的资源，包括 PrintWriter 和 clientSocket。
```java
finally {
    if (pw != null) {
        pw.close();
...
首先，在终端中找到服务器代码所在的目录，并启动服务器：
```sh
cd /f:/desktop/文件/6-专业课/计算机网络/课程设计/WebServer
java Server.Server
```
可以看到下面的输出，此时服务器线程被阻塞，一直在等待客户端程序的连接。当连接成功后，服务器程序正常运行，并返回客户端套接字clientSocket。
![连接测试1](../docs/images/image-服务器与客户端连接测试1.png)

### 启动客户端

打开另一个终端，找到客户端代码所在的目录，并启动客户端：
```sh
cd /f:/desktop/文件/6-专业课/计算机网络/课程设计/WebServer
java Client.Client
```
可以看到以下输出，
![连接测试2](../docs/images/image-服务器与客户端连接测试2.png)

### 验证服务器输出

在服务器的终端中，可以看到客户端发送的消息 Hello, Server，表示服务器已成功接收到客户端的数据。

![连接测试3](../docs/images/image-服务器与客户端连接测试3.png)

### 关闭连接

在客户端发送数据后，客户端和服务器都会关闭各自的连接。